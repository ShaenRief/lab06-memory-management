...
public class Scene : SKScene {
            ...
            if let collider = object.collider {
                ...
                for j in i+1..<objects.count {
                    ...
                    if let otherCollider = otherObject.collider {
                        ...
                        // Check for collision
                        if collider.inCollisionWith(other: otherCollider)  {
                            // If in collision, then send
                            // collidedWith message to both
                            // objects
                            object.collidedWith(other: otherObject)
                            otherObject.collidedWith(other: object)
                        }
                    }
                }
            }
    ...
    */
    public func addGameObject(object: GameObject) {
        root.addChild(object: object)
        if let collider=object.collider {
            object.scene?.registerGameObject(object: collider)
        }
    }    
