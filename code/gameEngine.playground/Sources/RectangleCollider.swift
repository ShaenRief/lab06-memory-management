    ...
    override public func start() {
        if let scene = self.scene {
            if scene.debug {
                ...
                let shape = SKShapeNode(rectOf: CGSize(width: self.width, height: self.height))
                shape.strokeColor = NSColor.green
                self.render = shape
                scene.registerGameObject(object: self)
            }
        }
    }
    ...
    */
    override public func inCollisionWith(other: Collider) -> Bool {
        ...
        if other is RectangleCollider {
            ...
            return rect.intersects(rectOther)
            ...
            if EdgeIntersectCircle(p0: e1, e2, circleCentre, radius) {
                return true
            }
            
            // Check if top horizontal edge intersects the circle collider
            if EdgeIntersectCircle(p0: e2, e3, circleCentre, radius){
                return true
            }
            
            // Check if right vertical edge intersects the circle collider
            if EdgeIntersectCircle(p0: e3, e4, circleCentre, radius) {
                return true
            }
            
            // Check if bottom horizontal edge intersects the circle collider
            if EdgeIntersectCircle(p0: e4, e1, circleCentre, radius) {
                return true
            }
        ...
        return super.inCollisionWith(other: other)
    }
